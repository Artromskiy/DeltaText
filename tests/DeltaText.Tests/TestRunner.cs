...
internal static class TestRunner
{
    ...
    private static void ShapingLayoutInvariants()
    {
        ...
        Check(shaped.TextLengthUtf16 == text.Length, "layout source length changed");
        Check(shaped.Runs.Length > 0, "layout produced no runs");

        var totalAdvance = 0f;
        for (var runIndex = 0; runIndex < shaped.Runs.Length; runIndex++)
        {
            var run = shaped.Runs.Span[runIndex];
            Check(run.SourceRange.StartUtf16 >= 0 && run.SourceRange.EndUtf16 <= text.Length,
                "layout run escaped source bounds");
            Check(run.Glyphs.Length > 0, "layout produced an empty run");
            Check(float.IsFinite(run.AdvanceX) && float.IsFinite(run.AdvanceY), "run advance is not finite");
            Check(float.IsFinite(run.Bounds.Left) && float.IsFinite(run.Bounds.Top)
                && float.IsFinite(run.Bounds.Right) && float.IsFinite(run.Bounds.Bottom),
                "run bounds are not finite");
            totalAdvance += run.AdvanceX;

            for (var glyphIndex = 0; glyphIndex < run.Glyphs.Length; glyphIndex++)
            {
                var glyph = run.Glyphs.Span[glyphIndex];
                Check(glyph.ClusterUtf16 >= run.SourceRange.StartUtf16
                    && glyph.ClusterUtf16 < run.SourceRange.EndUtf16,
                    "glyph cluster escaped its run range");
                Check(glyph.ClusterUtf16 >= text.Length
                    || !char.IsLowSurrogate(text[glyph.ClusterUtf16]),
                    "glyph cluster points into a UTF-16 surrogate pair");
                Check(float.IsFinite(glyph.AdvanceX) && float.IsFinite(glyph.AdvanceY)
                    && float.IsFinite(glyph.OffsetX) && float.IsFinite(glyph.OffsetY),
                    "glyph positioning is not finite");
            }
        }

        Check(float.IsFinite(totalAdvance) && totalAdvance > 0, "layout has no horizontal advance");
    }
    ...
    private static void UnicodeSmoke()
    {
        ...
        var request = new TextShapeRequest(text.AsMemory(), 28, new[] { latin, arabic });
        var shaped = service.Shape(request);
        Check(shaped.TextLengthUtf16 == text.Length, "Unicode smoke changed source length");
        Check(shaped.Runs.Length > 0, "Unicode smoke produced no runs");
        for (var runIndex = 0; runIndex < shaped.Runs.Length; runIndex++)
        {
            var run = shaped.Runs.Span[runIndex];
            Check(run.SourceRange.StartUtf16 >= 0 && run.SourceRange.EndUtf16 <= text.Length,
                "Unicode smoke produced an out-of-range source run");
            Check(run.Glyphs.Length > 0, "Unicode smoke produced a zero-glyph run");
        }

        var rendered = new CpuTextRenderer(service).Render(request);
        Check(rendered.IsEmpty is false || shaped.Runs.Length > 0,
            "Unicode smoke unexpectedly lost all render output");
    }
    ...
    private static void GlyphImageBaselinePlacement()
    {
        ...
        Check(glyphs.Length == 3, "baseline fixture was shaped into an unexpected glyph count");

        var letter = service.GenerateGlyphImage(new GlyphImageRequest(
            font,
            glyphs[0].GlyphId,
            96,
            GlyphImageMode.Coverage));
        var dash = service.GenerateGlyphImage(new GlyphImageRequest(
            font,
            glyphs[1].GlyphId,
            96,
            GlyphImageMode.Coverage));
        Check(dash.PlaneBounds.Top > letter.PlaneBounds.Top,
            "dash top was normalized instead of preserving its baseline position");
        Check(dash.PlaneBounds.Bottom < letter.PlaneBounds.Bottom,
            "dash bottom was normalized instead of preserving its baseline position");
        Check(letter.PlaneBounds.Bottom < 0 && dash.PlaneBounds.Bottom < 0,
            "horizontal glyph images are not baseline-relative");
    }

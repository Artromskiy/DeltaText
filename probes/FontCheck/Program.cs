...
const string expectedIgrunokSha256 = "2743db6b21c637cb899f73a105731d1dcd9147346fc454e12b76eba4423cd1fd";
...
try
{
    ...
    var nativeCorpus = NativeConformance.Run(
        service,
        font,
        options.FontPath,
        fontBytes,
        options.OutputDirectory,
        options.NativeCorpusCount,
        options.SkipNative);
    ...
    if (nativeCorpus.Supported)
    {
        Console.WriteLine($"  Native corpus: {nativeCorpus.CaseCount} cases / {nativeCorpus.SizeCount} sizes, "
            + $"DeltaText↔CoreText exact frames={nativeCorpus.DeltaTextVsCoreText.ExactCaseCount}/"
            + $"{nativeCorpus.DeltaTextVsCoreText.CaseCount}, "
            + $"pixel mismatch={nativeCorpus.DeltaTextVsCoreText.MismatchedPixels:N0}, "
            + $"MAE={nativeCorpus.DeltaTextVsCoreText.MeanAbsoluteError:0.00}, "
            + $"P95={nativeCorpus.DeltaTextVsCoreText.P95Error}");
        Console.WriteLine($"  Native/ImageSharp alpha: {nativeCorpus.DeltaTextVsImageSharp.CaseCount} samples "
            + $"(every {nativeCorpus.ImageSharpSampleStride} cases), "
            + $"DeltaText MAE={nativeCorpus.DeltaTextVsImageSharp.MeanAbsoluteError:0.00}, "
            + $"CoreText MAE={nativeCorpus.CoreTextVsImageSharp.MeanAbsoluteError:0.00}");
        Console.WriteLine($"  Native first mismatch: case={nativeCorpus.FirstMismatchCase}, "
            + $"size={nativeCorpus.FirstMismatchPixelsPerEm:0.###}px, text='{nativeCorpus.FirstMismatchText}'");
    }
    else
    {
        Console.WriteLine($"  Native corpus: NOT RUN ({nativeCorpus.SkipReason})");
    }
...
static void RunShapeOnly(CheckOptions options)
{
    const int repeatCount = 100;
    const string shapeText = "R";
    Require(File.Exists(options.FontPath), $"Shape-check font was not found: {options.FontPath}");
    var fontBytes = File.ReadAllBytes(options.FontPath);
    var actualFontSha256 = Convert.ToHexString(SHA256.HashData(fontBytes)).ToLowerInvariant();
    ...
    try
    {
        var expected = CaptureShape(service, font, shapeText, options.ShapePixelsPerEm);
        Require(expected.GlyphId == 54 && expected.ClusterUtf16 == 0,
            $"Unexpected Igrunok R identity: {expected}.");
        if (options.ShapePixelsPerEm == 70f)
        {
            Require(expected.AdvanceX == 32.62f && expected.AdvanceY == 0f
                && expected.OffsetX == -0.70000005f && expected.OffsetY == 0f,
                $"Unexpected Igrunok R metrics: {expected}.");
        }
        for (var iteration = 0; iteration < repeatCount; iteration++)
        {
            var actual = CaptureShape(service, font, shapeText, options.ShapePixelsPerEm);
        ...
        try
        {
            var fresh = CaptureShape(freshService, freshFont, shapeText, options.ShapePixelsPerEm);
            Require(fresh == expected,
                $"Shaping changed between service instances. Expected {expected}, got {fresh}.");
        }
        finally
        {
            freshService.CloseFont(freshFont);
        }

        RenderOutputSummary? render = options.RenderPng
            ? RenderShapeComparison(service, font, fontBytes, options.FontPath, options.ShapePixelsPerEm, options.OutputDirectory)
            : null;

        Directory.CreateDirectory(options.OutputDirectory);
        var reportPath = Path.Combine(options.OutputDirectory, "Igrunok-R-shaping.json");
        var report = new
        {
            font = Path.GetFileName(options.FontPath),
            fontSha256 = Convert.ToHexString(SHA256.HashData(fontBytes)).ToLowerInvariant(),
            text = shapeText,
            pixelsPerEm = options.ShapePixelsPerEm,
            repeatCount,
            freshServiceMatch = true,
            snapshot = expected,
            render,
        };
        File.WriteAllText(reportPath, JsonSerializer.Serialize(report, new JsonSerializerOptions { WriteIndented = true }));

        Console.WriteLine("PASS FontCheck (shaping-only)");
        Console.WriteLine($"  font: {options.FontPath}");
        Console.WriteLine($"  text: '{shapeText}' at {options.ShapePixelsPerEm:0.###} px/em");
        Console.WriteLine($"  glyphId={expected.GlyphId}, cluster={expected.ClusterUtf16}, "
            + $"advance=({expected.AdvanceX:0.######}, {expected.AdvanceY:0.######}), "
            + $"offset=({expected.OffsetX:0.######}, {expected.OffsetY:0.######})");
        Console.WriteLine($"  repeats: {repeatCount}, fresh service: exact match");
        if (render is { } renderSummary)
        {
            Console.WriteLine($"  shared canvas: {renderSummary.SharedWidth}x{renderSummary.SharedHeight}");
            Console.WriteLine($"  stage 1 CoreText: {renderSummary.Stage1CoreTextPath}");
            Console.WriteLine($"  stage 1 ImageSharp: {renderSummary.Stage1ImageSharpPath}");
            Console.WriteLine($"  stage 3 aligned: {renderSummary.AlignedWidth}x{renderSummary.AlignedHeight}");
            Console.WriteLine($"  stage 3 CoreText: {renderSummary.Stage3CoreTextPath}");
            Console.WriteLine($"  stage 3 DeltaText: {renderSummary.Stage3DeltaTextPath}");
            Console.WriteLine($"  stage 3 ImageSharp: {renderSummary.Stage3ImageSharpPath}");
            Console.WriteLine($"  stage 3 SixLabors flipped: {renderSummary.Stage3ImageSharpFlippedPath}");
            Console.WriteLine($"  outline overlay: {renderSummary.OutlineOverlayPath}");
            Console.WriteLine($"  stage 3 CoreText↔DeltaText difference: {renderSummary.Stage3CoreTextDeltaTextDifferencePath}");
            Console.WriteLine($"  stage 3 DeltaText↔ImageSharp difference: {renderSummary.Stage3DeltaTextImageSharpDifferencePath}");
            Console.WriteLine($"  shaped glyph ↔ reference glyph: {renderSummary.ShapedGlyphId}/{renderSummary.ReferenceGlyphId}, "
                + $"grapheme={renderSummary.ReferenceGraphemeIndex}");
            Console.WriteLine($"  alpha diff CoreText↔DeltaText: {DescribePixelDifference(renderSummary.CoreTextDeltaTextDifference)}");
            Console.WriteLine($"  alpha diff DeltaText↔ImageSharp: {DescribePixelDifference(renderSummary.DeltaTextImageSharpDifference)}");
            Console.WriteLine($"  outline commands SixLabors/CoreText: {renderSummary.Outline.SixLaborsCommandCount}/"
                + $"{renderSummary.Outline.CoreTextCommandCount}, "
                + $"sampled points={renderSummary.Outline.SixLaborsPointCount}/"
                + $"{renderSummary.Outline.CoreTextPointCount}, "
                + $"same kinds={renderSummary.Outline.SameCommandKinds}, "
                + $"direct max error={renderSummary.Outline.DirectMaximumError:0.######}, "
                + $"mirrored max error={renderSummary.Outline.MirroredMaximumError:0.######}, "
                + $"best={renderSummary.Outline.BestTransform}");
            Console.WriteLine($"  Skia: glyphId={renderSummary.Skia.GlyphId}, "
                + $"advance={renderSummary.Skia.AdvanceX:0.######} "
                + $"(DeltaText={renderSummary.Skia.DeltaTextAdvanceX:0.######}, "
                + $"error={renderSummary.Skia.AdvanceDifference:0.######}), "
                + $"path bounds={renderSummary.Skia.Path.Bounds}, "
                + $"commands={renderSummary.Skia.Path.CommandCount}, "
                + $"points={renderSummary.Skia.Path.PointCount}, "
                + $"hash=0x{renderSummary.Skia.Path.Hash:X16}");
            Console.WriteLine($"  Skia path vs SixLabors: best={renderSummary.Skia.SixLaborsOutline.BestTransform}, "
                + $"direct max={renderSummary.Skia.SixLaborsOutline.DirectMaximumError:0.######}, "
                + $"mirrored max={renderSummary.Skia.SixLaborsOutline.MirroredMaximumError:0.######}");
            Console.WriteLine($"  Skia path vs CoreText: best={renderSummary.Skia.CoreTextOutline.BestTransform}, "
                + $"direct max={renderSummary.Skia.CoreTextOutline.DirectMaximumError:0.######}, "
                + $"mirrored max={renderSummary.Skia.CoreTextOutline.MirroredMaximumError:0.######}");
            Console.WriteLine($"  stage 4 Skia: {renderSummary.Skia.BitmapPath}");
            Console.WriteLine($"  stage 4 Skia flipped: {renderSummary.Skia.FlippedBitmapPath}");
            Console.WriteLine($"  outline commands: {renderSummary.OutlineCommandsPath}");
            Console.WriteLine($"  alpha diff Skia↔CoreText: {DescribePixelDifference(renderSummary.Skia.SkiaCoreTextDifference)}");
            Console.WriteLine($"  alpha diff Skia flipped↔CoreText: {DescribePixelDifference(renderSummary.Skia.SkiaCoreTextFlippedDifference)}");
            Console.WriteLine($"  alpha diff Skia↔SixLabors: {DescribePixelDifference(renderSummary.Skia.SkiaSixLaborsDifference)}");
            Console.WriteLine($"  alpha diff Skia↔DeltaText: {DescribePixelDifference(renderSummary.Skia.SkiaDeltaTextDifference)}");
        }
...
{
    const string text = "R";
    var request = new TextShapeRequest(text.AsMemory(), pixelsPerEm, new[] { font }, TextDirection.LeftToRight);
    var shaped = service.Shape(request);
    var frame = new CpuTextRenderer(service).Render(request, new CpuTextRenderOptions(
        GlyphImageMode.Coverage,
        0,
        new Rgba32(255, 255, 255, 255)));
    Require(!frame.IsEmpty, "The shaped R produced an empty frame for CoreText rendering.");
    Require(NativeCoreTextRenderer.IsSupported, "CoreText rendering is available only on macOS.");

    using var nativeRenderer = new NativeCoreTextRenderer(fontPath);
    var reference = ReferenceFontRenderer.Render(fontBytes, text, pixelsPerEm);
    Require(reference.Width > 0 && reference.Height > 0,
        "The ImageSharp reference produced an empty R bitmap.");
    Require(shaped.Runs.Length == 1 && shaped.Runs.Span[0].Glyphs.Length == 1,
        "The DeltaText R fixture did not produce one shaped glyph.");
    Require(reference.Glyphs.Length == 1,
        $"The SixLabors reference R fixture produced {reference.Glyphs.Length} glyphs instead of one.");
    var shapedGlyph = shaped.Runs.Span[0].Glyphs.Span[0];
    var referenceGlyph = reference.Glyphs[0];
    Require(referenceGlyph.GlyphId == shapedGlyph.GlyphId,
        $"Shaping glyph IDs differ: DeltaText={shapedGlyph.GlyphId}, SixLabors={referenceGlyph.GlyphId}.");
    Require(referenceGlyph.GraphemeIndex == 0,
        $"The SixLabors reference R fixture has an unexpected grapheme index: {referenceGlyph.GraphemeIndex}.");
    var outlineData = OutlineComparison.Capture(fontBytes, fontPath, shapedGlyph.GlyphId, pixelsPerEm);
    var outline = OutlineComparison.Summarize(outlineData);
    var skia = SkiaProbe.Capture(fontBytes, text, pixelsPerEm);
    Require(skia.GlyphId == shapedGlyph.GlyphId,
        $"Skia glyph ID differs: DeltaText={shapedGlyph.GlyphId}, Skia={skia.GlyphId}.");
    var skiaSixLaborsOutline = OutlineComparison.ComparePair(
        shapedGlyph.GlyphId,
        outlineData.SixLaborsCommands,
        skia.Commands);
    var skiaCoreTextOutline = OutlineComparison.ComparePair(
        shapedGlyph.GlyphId,
    ...
    var native = nativeRenderer.Render(shaped, canvasBounds, canvasWidth, canvasHeight, pixelsPerEm);

    Directory.CreateDirectory(outputDirectory);
    var background = new Rgba32(32, 32, 32, 255);
    var referenceCanvas = new byte[checked(canvasWidth * canvasHeight * 4)];
    var referencePixels = ToRgbaBytes(reference.Pixels);
    for (var row = 0; row < reference.Height; row++)
    {
        referencePixels.AsSpan(row * reference.Width * 4, reference.Width * 4)
            .CopyTo(referenceCanvas.AsSpan(
                checked(((reference.Top - canvasTop + row) * canvasWidth + reference.Left - canvasLeft) * 4),
                reference.Width * 4));
    }

    var nativePreview = CompositeOnBackground(native.Pixels, background);
    var referencePreview = CompositeOnBackground(referenceCanvas, background);
    var stage1CoreTextPath = Path.Combine(outputDirectory, "Igrunok-R-stage1-CoreText-shared.png");
    var stage1ImageSharpPath = Path.Combine(outputDirectory, "Igrunok-R-stage1-ImageSharp-shared.png");
    SaveImageSharpPng(stage1CoreTextPath, native.Width, native.Height, nativePreview);
    SaveImageSharpPng(stage1ImageSharpPath, canvasWidth, canvasHeight, referencePreview);

    var nativeBounds = GetAlphaBounds(native.Pixels, native.Width, native.Height);
    ...
    var alignedDeltaText = CropToPadded(frame.Pixels.Span, frame.Width, deltaTextBounds, alignedWidth, alignedHeight, 32);
    var alignedSkia = CropToPadded(skia.Pixels, skia.Width, skiaBounds, alignedWidth, alignedHeight, 32);
    var alignedSkiaFlipped = FlipRows(alignedSkia, alignedWidth, alignedHeight);
    var stage3CoreTextPath = Path.Combine(outputDirectory, "Igrunok-R-stage3-CoreText-aligned.png");
    var stage3ImageSharpPath = Path.Combine(outputDirectory, "Igrunok-R-stage3-ImageSharp-aligned.png");
    var stage3ImageSharpFlippedPath = Path.Combine(outputDirectory, "Igrunok-R-stage3-SixLabors-flipped.png");
    var stage3DeltaTextPath = Path.Combine(outputDirectory, "Igrunok-R-stage3-DeltaText-aligned.png");
    var stage3CoreTextDeltaTextDifferencePath = Path.Combine(outputDirectory, "Igrunok-R-stage3-difference-CoreText-DeltaText.png");
    var stage3DeltaTextImageSharpDifferencePath = Path.Combine(outputDirectory, "Igrunok-R-stage3-difference-DeltaText-ImageSharp.png");
    SaveImageSharpPng(stage3CoreTextPath, alignedWidth, alignedHeight,
        CompositeOnBackground(alignedNative, background));
    SaveImageSharpPng(stage3ImageSharpPath, alignedWidth, alignedHeight,
        CompositeOnBackground(alignedReference, background));
    SaveImageSharpPng(stage3ImageSharpFlippedPath, alignedWidth, alignedHeight,
        CompositeOnBackground(FlipRows(alignedReference, alignedWidth, alignedHeight), background));
    SaveImageSharpPng(stage3DeltaTextPath, alignedWidth, alignedHeight,
        CompositeOnBackground(alignedDeltaText, background));
    var outlineOverlayPath = Path.Combine(outputDirectory, "Igrunok-R-outline-overlay-CoreText-SixLabors.png");
    SaveImageSharpPng(outlineOverlayPath, 768, 768, OutlineComparison.RenderOverlay(outlineData, 768, 768));
    var outlineCommandsPath = Path.Combine(outputDirectory, "Igrunok-R-outline-commands.txt");
    File.WriteAllText(
        outlineCommandsPath,
        "SixLabors\n========\n"
        + OutlineComparison.FormatCommands(outlineData.SixLaborsCommands)
        + "\nCoreText\n========\n"
        + OutlineComparison.FormatCommands(outlineData.CoreTextCommands)
        + "\nSkiaSharp\n=========\n"
        + OutlineComparison.FormatCommands(skia.Commands));
    SaveImageSharpPng(stage3CoreTextDeltaTextDifferencePath, alignedWidth, alignedHeight,
        CreateDifferencePreview(alignedNative, alignedDeltaText, background));
    SaveImageSharpPng(stage3DeltaTextImageSharpDifferencePath, alignedWidth, alignedHeight,
        CreateDifferencePreview(alignedDeltaText, alignedReference, background));
    var stage3CoreTextImageSharpDifferencePath = Path.Combine(outputDirectory, "Igrunok-R-stage3-difference-CoreText-ImageSharp.png");
    SaveImageSharpPng(stage3CoreTextImageSharpDifferencePath, alignedWidth, alignedHeight,
        CreateDifferencePreview(alignedNative, alignedReference, background));
    var stage4SkiaPath = Path.Combine(outputDirectory, "Igrunok-R-stage4-Skia.png");
    var stage4SkiaFlippedPath = Path.Combine(outputDirectory, "Igrunok-R-stage4-Skia-flipped.png");
    var stage4SkiaCoreTextDifferencePath = Path.Combine(outputDirectory, "Igrunok-R-stage4-difference-Skia-CoreText.png");
    var stage4SkiaSixLaborsDifferencePath = Path.Combine(outputDirectory, "Igrunok-R-stage4-difference-Skia-SixLabors.png");
    var stage4SkiaDeltaTextDifferencePath = Path.Combine(outputDirectory, "Igrunok-R-stage4-difference-Skia-DeltaText.png");
    SaveImageSharpPng(stage4SkiaPath, alignedWidth, alignedHeight,
        CompositeOnBackground(alignedSkia, background));
    SaveImageSharpPng(stage4SkiaFlippedPath, alignedWidth, alignedHeight,
        CompositeOnBackground(alignedSkiaFlipped, background));
    SaveImageSharpPng(stage4SkiaCoreTextDifferencePath, alignedWidth, alignedHeight,
        CreateDifferencePreview(alignedSkia, alignedNative, background));
    SaveImageSharpPng(stage4SkiaSixLaborsDifferencePath, alignedWidth, alignedHeight,
        CreateDifferencePreview(alignedSkia, alignedReference, background));
    SaveImageSharpPng(stage4SkiaDeltaTextDifferencePath, alignedWidth, alignedHeight,
        CreateDifferencePreview(alignedSkia, alignedDeltaText, background));
    var coreTextDeltaTextDifference = CompareAlpha(alignedNative, alignedDeltaText);
    var deltaTextImageSharpDifference = CompareAlpha(alignedDeltaText, alignedReference);
    var skiaCoreTextDifference = CompareAlpha(alignedSkia, alignedNative);
    var skiaCoreTextFlippedDifference = CompareAlpha(alignedSkiaFlipped, alignedNative);
    var skiaSixLaborsDifference = CompareAlpha(alignedSkia, alignedReference);
    var skiaDeltaTextDifference = CompareAlpha(alignedSkia, alignedDeltaText);
    return new RenderOutputSummary(
        native.Width,
        native.Height,
        stage1CoreTextPath,
        stage1ImageSharpPath,
        alignedWidth,
        alignedHeight,
        stage3CoreTextPath,
        stage3ImageSharpPath,
        stage3ImageSharpFlippedPath,
        stage3DeltaTextPath,
        stage3CoreTextDeltaTextDifferencePath,
        stage3DeltaTextImageSharpDifferencePath,
        shapedGlyph.GlyphId,
        referenceGlyph.GlyphId,
        referenceGlyph.GraphemeIndex,
        coreTextDeltaTextDifference,
        deltaTextImageSharpDifference,
        stage3CoreTextImageSharpDifferencePath,
        outline,
        outlineOverlayPath,
        outlineCommandsPath,
        new SkiaComparisonSummary(
            skia.GlyphId,
            skia.AdvanceX,
            shapedGlyph.AdvanceX,
            skiaAdvanceDifference,
            skia.Path,
            skia.Width,
            skia.Height,
            skia.AlphaHash,
            stage4SkiaPath,
            stage4SkiaFlippedPath,
            stage4SkiaCoreTextDifferencePath,
            stage4SkiaSixLaborsDifferencePath,
            stage4SkiaDeltaTextDifferencePath,
            skiaCoreTextDifference,
            skiaCoreTextFlippedDifference,
            skiaSixLaborsDifference,
            skiaDeltaTextDifference,
            skiaSixLaborsOutline,
            skiaCoreTextOutline));
}

static byte[] FlipRows(ReadOnlySpan<byte> source, int width, int height)
{
    var rowBytes = checked(width * 4);
    Require(source.Length == checked(rowBytes * height), "The image payload has an invalid RGBA size.");
    var flipped = new byte[source.Length];
    for (var row = 0; row < height; row++)
    {
        source.Slice(row * rowBytes, rowBytes)
            .CopyTo(flipped.AsSpan((height - row - 1) * rowBytes, rowBytes));
    }

    return flipped;
}

static ImageBounds GetAlphaBounds(ReadOnlySpan<byte> rgba, int width, int height)
{
    var left = width;
    var top = height;
    var right = 0;
    ...
    Require(right > left && bottom > top, "The rendered glyph has no visible alpha bounds.");
    return new ImageBounds(left, top, right, bottom);
}

static byte[] CropToPadded(
    ReadOnlySpan<byte> source,
    int sourceWidth,
    ImageBounds sourceBounds,
    int destinationWidth,
    int destinationHeight,
    int padding)
{
    var destination = new byte[checked(destinationWidth * destinationHeight * 4)];
    for (var row = 0; row < sourceBounds.Height; row++)
    {
        source.Slice(
                ((sourceBounds.Top + row) * sourceWidth + sourceBounds.Left) * 4,
                sourceBounds.Width * 4)
            .CopyTo(destination.AsSpan((padding + row) * destinationWidth * 4 + padding * 4, sourceBounds.Width * 4));
    }

    return destination;
}

static TextBounds GetShapedBounds(ShapedText shaped)
{
    ...
    Require(float.IsFinite(left) && float.IsFinite(top)
        ...
        "The shaped R has no finite bounds.");
    return new TextBounds(left, top, right, bottom);
}

static byte[] CreateDifferencePreview(
    ReadOnlySpan<byte> first,
    ReadOnlySpan<byte> second,
    Rgba32 background)
{
    Require(first.Length == second.Length, "The render previews have different canvas sizes.");
...
static PixelDifferenceSummary CompareAlpha(ReadOnlySpan<byte> first, ReadOnlySpan<byte> second)
{
    Require(first.Length == second.Length && first.Length % 4 == 0,
        "The render images have incompatible RGBA payloads.");
    var comparedPixels = first.Length / 4;
    ...
    return new PixelDifferenceSummary(
        comparedPixels,
        differentPixels,
        maximumDifference,
        (double)totalDifference / comparedPixels);
}

static string DescribePixelDifference(PixelDifferenceSummary difference)
    => $"{difference.DifferentPixels}/{difference.ComparedPixels} pixels, "
        + $"max={difference.MaximumAlphaDifference}, mean={difference.MeanAbsoluteAlphaDifference:0.###}";

static void SaveImageSharpPng(string path, int width, int height, ReadOnlySpan<byte> rgba)
{
    var pixels = new ImageRgba32[checked(width * height)];
    Require(rgba.Length == checked(pixels.Length * 4), "RGBA image payload has an invalid length.");
    for (var index = 0; index < pixels.Length; index++)
    {
        var sourceIndex = index * 4;
        pixels[index] = new ImageRgba32(
            rgba[sourceIndex],
            rgba[sourceIndex + 1],
            rgba[sourceIndex + 2],
            rgba[sourceIndex + 3]);
    ...
    using var image = SixLabors.ImageSharp.Image.LoadPixelData<ImageRgba32>(pixels, width, height);
    image.SaveAsPng(path);
}

static byte[] ToRgbaBytes(ImageRgba32[] pixels)
{
    var rgba = new byte[checked(pixels.Length * 4)];
    for (var index = 0; index < pixels.Length; index++)
    {
        var pixel = pixels[index];
        var destinationIndex = index * 4;
        rgba[destinationIndex] = (byte)(pixel.R * pixel.A / byte.MaxValue);
        rgba[destinationIndex + 1] = (byte)(pixel.G * pixel.A / byte.MaxValue);
        rgba[destinationIndex + 2] = (byte)(pixel.B * pixel.A / byte.MaxValue);
        rgba[destinationIndex + 3] = pixel.A;
    }
    ...
    float pixelsPerEm)
{
    ...
    Require(shaped.TextLengthUtf16 == text.Length, "Shape-only result has an invalid source length.");
    Require(shaped.Runs.Length == 1, $"Expected one run for '{text}', got {shaped.Runs.Length}.");

    var run = shaped.Runs.Span[0];
    Require(run.SourceRange == new TextRange(0, text.Length), "Shape-only run does not cover the source.");
    Require(run.Direction == TextDirection.LeftToRight && run.BidiLevel == 0,
        "Shape-only run did not resolve to the requested LTR direction.");
    Require(run.Glyphs.Length == 1, $"Expected one glyph for '{text}', got {run.Glyphs.Length}.");

    var glyph = run.Glyphs.Span[0];
    Require(glyph.GlyphId != 0, "Shape-only glyph resolved to the missing-glyph ID.");
    Require(glyph.ClusterUtf16 == 0, "Shape-only glyph does not start at UTF-16 cluster zero.");
    Require(float.IsFinite(run.AdvanceX) && float.IsFinite(run.AdvanceY)
        && float.IsFinite(run.Bounds.Left) && float.IsFinite(run.Bounds.Top)
        && float.IsFinite(run.Bounds.Right) && float.IsFinite(run.Bounds.Bottom)
        && float.IsFinite(glyph.AdvanceX) && float.IsFinite(glyph.AdvanceY)
        && float.IsFinite(glyph.OffsetX) && float.IsFinite(glyph.OffsetY),
        "Shape-only result contains non-finite metrics.");

    return new ShapeSnapshot(
        shaped.TextLengthUtf16,
        run.SourceRange,
        run.Direction,
        run.BidiLevel,
        run.PixelsPerEm,
...
internal readonly record struct ShapeSnapshot(
    int TextLengthUtf16,
...
internal readonly record struct ImageBounds(int Left, int Top, int Right, int Bottom)
{
...
internal readonly record struct RenderOutputSummary(
    int SharedWidth,
    int SharedHeight,
    string Stage1CoreTextPath,
    string Stage1ImageSharpPath,
    int AlignedWidth,
    int AlignedHeight,
    string Stage3CoreTextPath,
    string Stage3ImageSharpPath,
    string Stage3ImageSharpFlippedPath,
    string Stage3DeltaTextPath,
    string Stage3CoreTextDeltaTextDifferencePath,
    string Stage3DeltaTextImageSharpDifferencePath,
    uint ShapedGlyphId,
    ushort ReferenceGlyphId,
    int ReferenceGraphemeIndex,
    PixelDifferenceSummary CoreTextDeltaTextDifference,
    PixelDifferenceSummary DeltaTextImageSharpDifference,
    string Stage3CoreTextImageSharpDifferencePath,
    OutlineComparisonSummary Outline,
    string OutlineOverlayPath,
    string OutlineCommandsPath,
    SkiaComparisonSummary Skia);

internal readonly record struct SkiaComparisonSummary(
    ushort GlyphId,
    float AdvanceX,
    float DeltaTextAdvanceX,
    float AdvanceDifference,
    SkiaPathSummary Path,
    int BitmapWidth,
    int BitmapHeight,
    ulong BitmapAlphaHash,
    string BitmapPath,
    string FlippedBitmapPath,
    string CoreTextDifferencePath,
    string SixLaborsDifferencePath,
    string DeltaTextDifferencePath,
    PixelDifferenceSummary SkiaCoreTextDifference,
    PixelDifferenceSummary SkiaCoreTextFlippedDifference,
    PixelDifferenceSummary SkiaSixLaborsDifference,
    PixelDifferenceSummary SkiaDeltaTextDifference,
    OutlinePairSummary SixLaborsOutline,
    OutlinePairSummary CoreTextOutline);

internal readonly record struct PixelDifferenceSummary(
    int ComparedPixels,
    int DifferentPixels,
    int MaximumAlphaDifference,
    double MeanAbsoluteAlphaDifference);
...
internal sealed class CheckOptions
{
    private CheckOptions(
        ...
        int nativeCorpusCount,
        bool skipNative,
        bool shapeOnly,
        float shapePixelsPerEm,
        bool renderPng)
    {
        FontPath = fontPath;
        ...
        NativeCorpusCount = nativeCorpusCount;
        SkipNative = skipNative;
        ShapeOnly = shapeOnly;
        ShapePixelsPerEm = shapePixelsPerEm;
        RenderPng = renderPng;
    }
    ...
    internal int NativeCorpusCount { get; }

    internal bool SkipNative { get; }

    internal bool ShapeOnly { get; }

    internal float ShapePixelsPerEm { get; }

    internal bool RenderPng { get; }
    ...
    internal static CheckOptions Parse(string[] args)
    {
        ...
        var nativeCorpusCount = 2048;
        var skipNative = false;
        var shapeOnly = false;
        var shapePixelsPerEm = 70f;
        var renderPng = false;
        for (var i = 0; i < args.Length; i++)
        {
            switch (args[i])
            {
                ...
                case "--bidi-brackets":
                    ...
                    break;
                case "--native-corpus-count":
                    nativeCorpusCount = ParsePositiveInt(args, ref i, "--native-corpus-count");
                    break;
                case "--skip-native":
                    skipNative = true;
                    break;
                ...
                case "--render-png":
                    ...
                    break;
                case "--pixels-per-em":
                    shapePixelsPerEm = ParsePositiveFloat(args, ref i, "--pixels-per-em");
                    break;
                case "--help":
                    Console.WriteLine("Usage: dotnet run --project probes/FontCheck/FontCheck.csproj -c Release -- [--font PATH] [--luckiest-guy PATH] [--output DIRECTORY] [--bidi-corpus PATH] [--bidi-test PATH] [--bidi-brackets PATH] [--native-corpus-count COUNT] [--skip-native] [--shape-only --render-png --pixels-per-em 70]");
                    Environment.Exit(0);
                    break;
                default:
    ...
    private static float ParsePositiveFloat(string[] args, ref int index, string option)
    {
        var value = ReadValue(args, ref index, option);

...
internal sealed class MsdfGeometry
{
    ...
    private MsdfGeometry(int width, int height, MsdfEdge[] edges, MsdfEdgeGrid grid)
    {
        Width = width;
        Height = height;
        Edges = edges;
        ...
        [NotNullWhen(true)] out MsdfGeometry? geometry)
    {
        ...
        geometry = new MsdfGeometry(width, height, edges, MsdfEdgeGrid.Create(edges, width, height, distanceRange));
        return true;
    }
    ...
    private static float2 ToPixel(float2 point, Bounds bounds, float scale, int padding)
        => new(
            (point.x - bounds.Left) * scale + padding + 1,
            (bounds.Bottom - point.y) * scale + padding + 1);

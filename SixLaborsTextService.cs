...
public class SixLaborsTextService : ITextService
{
    ...
    public ShapedText Shape(in TextShapeRequest request)
    {
        ...
        lock (_gate)
        {
            ...
            {
                throw new InvalidOperationException("SixLabors returned inconsistent glyph layout and renderer output.");
            }
            ...
            for (var i = 0; i < metrics.Length; i++)
            {
                ...
                {
                    captured.Outline.Translate(-metric.Advance.X, -metric.Advance.Y);
    ...
    {
        private readonly List<ShapedGlyph> _glyphs = new();
        ...
        internal void Add(ShapedGlyph glyph, FontRectangle bounds, float advanceX, float advanceY)
        {
            _glyphs.Add(glyph);
            _advanceX += advanceX;
            _advanceY += advanceY;
